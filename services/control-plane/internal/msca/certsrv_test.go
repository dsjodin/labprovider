...
// The ReqID is needed for the seconds between certfnsh.asp and certnew.cer, and
// this listener outlives every one of them. Without a cap the map only grows.
func TestIssuedCertificatesAreCapped(t *testing.T) {
	h := &handler{byID: map[int][]byte{}}
	for i := 0; i < maxIssuedRetained*3; i++ {
		h.put([]byte{byte(i)})
	}
	if len(h.byID) > maxIssuedRetained {
		t.Errorf("retained %d issued certs, want at most %d", len(h.byID), maxIssuedRetained)
	}
	// The most recent must still be retrievable: that is the whole point of
	// keeping any.
	if _, ok := h.byID[h.seq]; !ok {
		t.Errorf("the newest ReqID %d was evicted", h.seq)
	}
	if _, ok := h.byID[1]; ok {
		t.Error("the oldest ReqID should have been evicted")
	}
}
