...
// Harbor's containers are the reason the filter looks at the compose project:
// prepare names four of them registry, registryctl, redis, and nginx, so a
// "harbor" filter matching on name alone would hide them.
func TestList_FiltersOnComposeProject(t *testing.T) {
	body := `[
		{"Id":"a1","Names":["/nginx"],"Image":"goharbor/nginx-photon:v2.13.0",
		 "State":"running","Status":"Up 2 hours","Created":1000,
		 "Labels":{"com.docker.compose.project":"harbor"}},
		{"Id":"b2","Names":["/redis"],"Image":"redis:7","State":"running",
		 "Status":"Up 1 hour","Created":900,"Labels":{}}
	]`
	c := newTestClient(t, func(w http.ResponseWriter, _ *http.Request) {
		_, _ = w.Write([]byte(body))
	})

	got, err := c.List(context.Background(), []string{"harbor"}, time.Unix(4600, 0))
	if err != nil {
		t.Fatal(err)
	}
	if len(got) != 1 || got[0].Name != "nginx" {
		t.Fatalf("filter kept %+v, want only Harbor's nginx", got)
	}
}
