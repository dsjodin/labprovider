...
func (d DNSSync) Deploy(ctx context.Context, rc *RunCtx) error {
	...
	if err := requireOutsideRuntime(env["DNS_SYNC_SECRETS_DIR"], runtime, "DNS_SYNC_SECRETS_DIR", "the operator's secrets"); err != nil {
		return err
	}
	...
	// Seed the live built-in file dns-sync re-reads each pass; OnConfigSaved
	// rewrites it on later edits so added/removed services converge without a
	// redeploy.
	if err := RefreshBuiltinRecordsFile(env); err != nil {
		return err
	}
...
// builtinRecordsFileName is the live built-in record file dns-sync re-reads
// each reconcile pass (newline-delimited fqdn=ip). It lives in DNS_SYNC_DIR,
// mounted read-only into the container.
const builtinRecordsFileName = "builtin-records"

// RefreshBuiltinRecordsFile writes the current built-in fqdn=ip set to the file
// mounted into dns-sync. It no-ops when dns-sync has not been deployed (its data
// dir is absent) or HOST_IPV4 is not yet derivable, so calling it on every
// config save is safe. The write is atomic (temp + rename) so a concurrent
// reconcile read never sees a partial file.
func RefreshBuiltinRecordsFile(env map[string]string) error {
	dir := env["DNS_SYNC_DIR"]
	if dir == "" || env["HOST_IPV4"] == "" {
		return nil
	}
	if _, err := os.Stat(dir); err != nil {
		return nil
	}
	value, err := builtinRecordsValue(env)
	if err != nil {
		return err
	}
	path := filepath.Join(dir, builtinRecordsFileName)
	tmp := path + ".tmp"
	if err := os.WriteFile(tmp, []byte(strings.ReplaceAll(value, ",", "\n")+"\n"), 0o644); err != nil {
		return err
	}
	_ = os.Chown(tmp, 1000, 1000)
