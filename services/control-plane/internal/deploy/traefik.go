...
func (t Traefik) Deploy(ctx context.Context, rc *RunCtx) error {
	...
	// Prefer the address this process is serving on: install.sh passes it in,
	// and routing to a config-file value the server never read would publish
	// dashboard.<domain> against a port nothing is listening on.
	addr := rc.eng.ListenAddr
	if addr == "" {
		addr = env["CONTROL_PLANE_ADDR"]
	}
	_, cpPort, err := net.SplitHostPort(addr)
	if err != nil {
		return fmt.Errorf("control plane listen address %q: %w", addr, err)
	}
	renderEnv := withDerived(env, map[string]string{"CONTROL_PLANE_PORT": cpPort})
	...
	if err := Render("traefik.yml.tpl", renderEnv, filepath.Join(runtime, "traefik.yml"), 0o644); err != nil {
		return err
	}
	if err := Render("traefik-dynamic.yml.tpl", renderEnv, filepath.Join(runtime, "dynamic", "dynamic.yml"), 0o644); err != nil {
		return err
	}
	if err := Render("docker-compose.traefik.yml.tpl", renderEnv, runtime+"/docker-compose.yml", 0o644); err != nil {
		return err
	}
