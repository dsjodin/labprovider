...
func (nb Netbox) Deploy(ctx context.Context, rc *RunCtx) error {
	...
	// NETBOX_SECRET_KEY's minimum length is enforced by the schema
	// (internal/envfile); the cross-field rules below cannot be.
	...
	cmp := Compose{Dir: env["NETBOX_DIR"], Project: "netbox", Out: func(line string) { rc.Log("%s", line) }}
...
func builtinServiceEntries(env map[string]string) []builtinService {
	return []builtinService{
		...
		{"smb", env["SAMBA_FQDN"], "tcp", env["SAMBA_PORT"]},
		{"kmip", env["KMIP_FQDN"], "tcp", env["KMIP_PORT"]},
		{"harbor", env["HARBOR_FQDN"], "tcp", env["HARBOR_HTTP_PORT"]},
	}
}
...
func builtinServiceFQDNs(env map[string]string) []string {
	var fqdns []string
	for _, key := range []string{"DNS_FQDN", "CA_FQDN", "DEPOT_FQDN", "KEYCLOAK_FQDN", "AUTHENTIK_FQDN", "ZITADEL_FQDN", "NETBOX_FQDN", "S3_FQDN", "SFTP_FQDN", "SYSLOG_FQDN", "NTP_FQDN", "MAILPIT_FQDN", "LLDAP_FQDN", "SAMBA_FQDN", "KMIP_FQDN", "HARBOR_FQDN", "CONTROL_PLANE_FQDN"} {
		if env[key] != "" {
