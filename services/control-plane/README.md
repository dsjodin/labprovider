...
1. **Certificates (step-ca)** - active certs with subject/SANs, provisioner,
   ...
   `SELECT`-only role, decoding the opaque cert blobs (see `docs/design/STEPCA_STORAGE.md`).
...
4. **Services** - one row per labprovider service: state, address, data
   directory, last deploy, and the containers backing it as detail. Joins the
   deploy registry, the recorded history, the managed config, and the live
   container list read from the Docker socket (mounted read-only), so
   "deployed then died" reads as stopped rather than ready. Containers that
   belong to no registry service are listed separately, and the read-only
   `--dashboard` deployment (no registry) falls back to the flat container
   table.
