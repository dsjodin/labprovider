...
func (l lazyCerts) List(ctx context.Context) ([]certs.Cert, error) {
	...
	for _, key := range []string{"CA_POSTGRES_RO_USER", "CA_POSTGRES_PORT", "CA_POSTGRES_DB"} {
		if env[key] == "" {
			return nil, fmt.Errorf("%s is not set in the configuration", key)
		}
	}
	// CA_POSTGRES_RO_PASSWORD is one of the generated secrets, so it is normally
	// empty in the config and lives in the secrets dir instead.
	password := envfile.LookupSecret(env, l.secretsDir(env), "CA_POSTGRES_RO_PASSWORD")
	if password == "" {
		return nil, fmt.Errorf("no CA_POSTGRES_RO_PASSWORD yet (set it in the configuration, or generated when ca deploys)")
	}
	dsn := (&url.URL{
		Scheme:   "postgresql",
	...
	r, err := certs.NewReader(dsn, password)
	if err != nil {
		return nil, err
	}
	return r.List(ctx)
...
func (l lazyIPAM) Fetch(ctx context.Context) (ipam.Overview, error) {
	...
	// NetBox serves plain HTTP on its loopback-published port (TLS is terminated
	// by Traefik on the FQDN's 443 entrypoint), so reach it over http - the same
	// scheme the deployer's newNetboxAPI uses for this host:port.
	c, err := ipam.New(fmt.Sprintf("http://%s:%s", env["NETBOX_FQDN"], env["NETBOX_PORT"]),
		token, l.caBundle(env), l.timeout)
	if err != nil {
		return ipam.Overview{}, err
	}
