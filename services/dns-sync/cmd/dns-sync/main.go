...
//
// With file set, the built-in set is re-read from that file every pass so a
// service added or removed in the control plane converges without a dns-sync
// redeploy; static (from DNS_SYNC_BUILTIN_RECORDS) is the initial/fallback set.
type sourceWithBuiltins struct {
	base     reconcile.Source
	static   []model.Record
	file     string
	logger   *slog.Logger
	lastGood []model.Record
}
...
func (s *sourceWithBuiltins) Desired(ctx context.Context) ([]model.Record, error) {
	...
	return append(desired, s.currentBuiltins()...), nil
}

// currentBuiltins returns the live built-in set. A read or parse error, or an
// empty file, keeps the last good set (or the static env set) instead of
// returning empty, so a transient failure never makes the reconciler full-sync
// away every service record.
func (s *sourceWithBuiltins) currentBuiltins() []model.Record {
	if s.file == "" {
		return s.static
	}
	...
	if err != nil {
		if !os.IsNotExist(err) {
			s.logger.Warn("read builtin records file; keeping last known set", "file", s.file, "err", err)
		}
		return s.fallback()
	}
	recs, err := parseBuiltinRecords(string(b))
	if err != nil {
		s.logger.Warn("parse builtin records file; keeping last known set", "file", s.file, "err", err)
		return s.fallback()
	}
	if len(recs) == 0 {
		return s.fallback()
...
func (s *sourceWithBuiltins) fallback() []model.Record {
	if s.lastGood != nil {
		return s.lastGood
	}
	return s.static
...
// parseBuiltinRecords parses fqdn=ipv4 pairs separated by commas or newlines
// (the env value is comma-separated; the live file is newline-separated) into
// A records.
...
func main() {
	...
	builtinRecordsFile := flag.String("builtin-records-file", os.Getenv("DNS_SYNC_BUILTIN_RECORDS_FILE"), "Path to a file of fqdn=ipv4 built-in records re-read every pass; the control plane rewrites it on config save so added/removed services converge without a redeploy")
	...
	staticBuiltins, err := parseBuiltinRecords(*builtinRecords)
	if err != nil {
		logger.Error("parse builtin records", "err", err)
		os.Exit(1)
	}
	if *builtinRecordsFile != "" || len(staticBuiltins) > 0 {
		source = &sourceWithBuiltins{base: nb, static: staticBuiltins, file: *builtinRecordsFile, logger: logger}
		logger.Info("built-in service records merged into desired set", "static_count", len(staticBuiltins), "live_file", *builtinRecordsFile)
	}
