...
### Changes
- Migrated service stacks behind Traefik with plain-HTTP backends: SeaweedFS S3 (path-style `Host` router), SFTPGo (admin UI; SFTP stays on its own port), the Technitium web console (DNS 53 and the HTTPS API 53443 stay published for consumers), and Keycloak (`--http-enabled`, `--proxy-headers=xforwarded`, portless `--hostname`; per-service cert dropped). Each keeps its port published on the loopback for deploy-time readiness, which is probed over HTTP, so deploy-time behavior is unchanged. Still on their own ports: depot, netbox, authentik, zitadel, and step-ca; the L4 services (DNS 53, NTP 123, syslog 514, SFTP 2022) always are.
